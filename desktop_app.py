...
uploading_files = set()  # Conjunto para controlar arquivos em upload para evitar duplicidade
...
def load_config():
    ...
    if os.path.exists(CONFIG_FILE):
        try:
            ...
            add_log(f"Erro ao carregar configurações (JSON inválido): {e}", "error")
            return False
...
def add_log(message, message_type="info"):
    ...
    if log_text: # Garante que o widget log_text existe
        ...
        log_text.insert(tk.END, formatted_message, message_type) # Insere com tag
        ...
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] {message_type.upper()}: {message}")
...
def update_monitor_status_ui():
    ...
    if monitor_status_label: # Garante que o widget existe
        if observer and observer.is_alive():
            monitor_status_label.config(text="Status do Monitor: Ativo", fg="green")
            start_monitor_button.config(state=tk.DISABLED)
            stop_monitor_button.config(state=tk.NORMAL)
        else:
            monitor_status_label.config(text="Status do Monitor: Inativo", fg="red")
    ...
    if monitoring_path_label: # Garante que o widget existe
        ...
        monitoring_path_label.config(text=f"Pasta de Monitoramento: {path_text}")
    ...
    if archive_folder_label: # Garante que o widget existe
        ...
        archive_folder_label.config(text=f"Pasta de Arquivamento Local: {path_text}")
...
class FileEventHandler(FileSystemEventHandler):
    ...
    def process_file(self, file_path):
        ...
        if file_path in uploading_files:
            add_log(f"Arquivo {os.path.basename(file_path)} já está em processo de upload, ignorando.", "warning")
            return
        ...
        threading.Thread(target=self._upload_and_archive_thread, args=(file_path,)).start()
    ...
    def _upload_and_archive_thread(self, file_path):
        ...
        try:
            ...
            if not os.path.exists(file_path):
                add_log(f"Erro: Arquivo não encontrado após espera: {file_path}", "error")
                return

            if not LOGGED_IN_USER_ID or not LOGGED_IN_USER_EMAIL:
                add_log("Erro: Nenhum usuário logado. Não é possível fazer upload.", "error")
                return

            add_log(f"Tentando upload de {os.path.basename(file_path)} para o servidor OpenPDF...")
            success = upload_file_to_server(file_path, LOGGED_IN_USER_ID, LOGGED_IN_USER_EMAIL)

            if success:
                add_log(f"Upload de {os.path.basename(file_path)} concluído com sucesso!", "success")
                archive_file_locally(file_path) # Move para a pasta de arquivamento local
            else:
                add_log(f"Falha no upload de {os.path.basename(file_path)}.", "error")
        except Exception as e:
            add_log(f"Erro inesperado ao processar arquivo {os.path.basename(file_path)}: {e}", "error")
        finally:
...
def start_monitoring():
    ...
    if not monitoring_path or not os.path.isdir(monitoring_path):
        add_log("Por favor, selecione uma pasta de monitoramento válida primeiro.", "warning")
        messagebox.showwarning("Configuração Necessária", "Por favor, selecione uma pasta de monitoramento válida antes de iniciar.")
        return

    if not local_archive_folder or not os.path.isdir(local_archive_folder):
        add_log("Por favor, selecione uma pasta de arquivamento local válida primeiro.", "warning")
        messagebox.showwarning("Configuração Necessária", "Por favor, selecione uma pasta de arquivamento local válida antes de iniciar.")
        return
    ...
    if not LOGGED_IN_USER_ID or not LOGGED_IN_USER_EMAIL:
        add_log("Nenhum usuário logado. Não é possível iniciar o monitoramento.", "error")
        messagebox.showerror("Erro de Login", "É necessário fazer login antes de iniciar o monitoramento.")
        return
...
def stop_monitoring():
    ...
    if observer:
        if observer.is_alive():
            ...
            if observer.is_alive():
                add_log("A thread do monitoramento não terminou após 5 segundos. Pode haver um problema.", "error")
            else:
...
def upload_file_to_server(file_path, user_id, user_email):
    ...
    if not os.path.exists(file_path):
        add_log(f"Erro de upload: Arquivo não existe no caminho: {file_path}", "error")
        return False

    file_name = os.path.basename(file_path)
    # Tenta determinar o caminho relativo se a pasta monitorada for um pai
    relative_path = os.path.relpath(os.path.dirname(file_path), monitoring_path)
    if relative_path == ".": # Se o arquivo estiver na raiz da pasta monitorada
        relative_path = "" # Envia como vazio para a rota Flask
    else:
        relative_path = relative_path.replace("\\", "/") # Garante barras corretas para URL
    ...
    try:
        with open(file_path, 'rb') as f: # Abrir o arquivo no try-except para garantir que seja fechado
            ...
            response = requests.post(f"{FLASK_SERVER_URL}/upload_desktop", files=files, data=data)
            response.raise_for_status() # Lança HTTPError para status de erro (4xx ou 5xx)

            try:
                response_data = response.json()
            except Exception:
                add_log(f"Erro ao enviar arquivo para o servidor: {response.text}", "error")
                return False

            if response_data.get('success'):
                add_log(f"Servidor retornou: {response_data.get('message', 'Upload bem-sucedido.')}", "success")
                return True
            else:
                add_log(f"Erro no servidor: {response_data.get('message', 'Upload falhou.')}", "error")
                return False
    except FileNotFoundError:
        add_log(f"Erro de upload: Arquivo não encontrado ao tentar abrir: {file_path}", "error")
        return False
    except requests.exceptions.ConnectionError:
        add_log(f"Erro de conexão: Não foi possível conectar ao servidor Flask em {FLASK_SERVER_URL}. Verifique se o servidor está rodando.", "error")
        messagebox.showerror("Erro de Conexão", f"Não foi possível conectar ao servidor Flask em {FLASK_SERVER_URL}. Verifique se ele está rodando.")
        return False
    except requests.exceptions.Timeout:
        add_log("Erro de timeout: O servidor Flask demorou muito para responder.", "error")
        messagebox.showerror("Erro de Timeout", "O servidor Flask demorou muito para responder.")
        return False
    except requests.exceptions.RequestException as e:
        add_log(f"Erro ao enviar arquivo para o servidor: {e}", "error")
        messagebox.showerror("Erro de Upload", f"Ocorreu um erro ao enviar o arquivo: {e}")
        return False
    except json.JSONDecodeError:
        add_log(f"Erro de JSON: Resposta inválida do servidor. Status: {response.status_code if 'response' in locals() else 'N/A'}, Conteúdo: {response.text if 'response' in locals() else 'N/A'}", "error")
        messagebox.showerror("Erro do Servidor", "O servidor enviou uma resposta inválida.")
        return False
    except Exception as e: # Captura qualquer outra exceção inesperada
        add_log(f"Erro inesperado no upload: {e}", "error")
        messagebox.showerror("Erro Inesperado", f"Ocorreu um erro inesperado durante o upload: {e}")
        return False
...
def archive_file_locally(file_path):
    ...
    try:
        ...
        dest_path = os.path.join(local_archive_folder, os.path.basename(file_path))
        ...
        add_log(f"Arquivo '{os.path.basename(file_path)}' movido para a pasta de arquivamento local: {dest_path}", "info")
        return True
    except Exception as e:
        add_log(f"Erro ao mover arquivo para arquivamento local: {e}", "error")
        messagebox.showerror("Erro de Arquivamento", f"Não foi possível arquivar o arquivo localmente: {e}")
        return False
...
def show_login_window():
    ...
    tk.Label(login_window, text="Login de Usuário", font=("Arial", 16)).pack(pady=10)
    ...
    def attempt_login():
        ...
        if response and response.get('success'):
            ...
            LOGGED_IN_USERNAME = response['user']['username']
            ...
            add_log(f"Falha no login: {response.get('message', 'Erro desconhecido.')}", "error")
            messagebox.showerror("Erro de Login", response.get('message', 'Falha ao conectar ou erro no servidor.'))
...
def send_login_request(email, password):
    ...
    try:
        response = requests.post(f"{FLASK_SERVER_URL}/login_desktop", json={"email": email, "password": password})
        response.raise_for_status() # Lança HTTPError para status de erro (4xx ou 5xx)
        ...
        add_log(f"Erro de JSON: Resposta inválida do servidor para login. Status: {response.status_code if 'response' in locals() else 'N/A'}, Conteúdo: {response.text if 'response' in locals() else 'N/A'}", "error")
...
def choose_archive_folder():
    ...
    if selected_path:
        ...
        update_monitor_status_ui()
        add_log(f"Pasta de arquivamento local selecionada: {local_archive_folder}")
    else:
...
def show_main_app_window():
    ...
    main_app_window.protocol("WM_DELETE_WINDOW", lambda: on_closing_main_app(main_app_window))
    ...
    tk.Label(main_frame, text=f"Bem-vindo(a), {LOGGED_IN_USERNAME}!", font=("Arial", 14, "bold")).pack(pady=5)
    tk.Label(main_frame, text="Monitor de Upload de Documentos", font=("Arial", 12)).pack(pady=5)

    # Status do Monitor
    monitor_status_label = tk.Label(main_frame, text="Status do Monitor: N/A", font=("Arial", 10, "bold"))
    monitor_status_label.pack(pady=5)

    # Caminhos selecionados
    monitoring_path_label = tk.Label(main_frame, text="Pasta de Monitoramento: N/A", wraplength=600)
    monitoring_path_label.pack(pady=2)
    archive_folder_label = tk.Label(main_frame, text="Pasta de Arquivamento Local: N/A", wraplength=600)
    ...
    tk.Button(path_buttons_frame, text="Selecionar Pasta de Monitoramento", command=choose_monitoring_path).pack(side=tk.LEFT, padx=5)
    tk.Button(path_buttons_frame, text="Selecionar Pasta de Arquivamento", command=choose_archive_folder).pack(side=tk.LEFT, padx=5)

    # Botões de Controle do Monitor
    monitor_buttons_frame = tk.Frame(main_frame)
    monitor_buttons_frame.pack(pady=10)
    start_monitor_button = tk.Button(monitor_buttons_frame, text="Iniciar Monitoramento", command=start_monitoring, state=tk.DISABLED)
    start_monitor_button.pack(side=tk.LEFT, padx=5)
    stop_monitor_button = tk.Button(monitor_buttons_frame, text="Parar Monitoramento", command=stop_monitoring, state=tk.DISABLED)
    stop_monitor_button.pack(side=tk.LEFT, padx=5)

    # Log de Atividade
    tk.Label(main_frame, text="Log de Atividade:", font=("Arial", 10, "bold")).pack(pady=5, anchor=tk.W)
    log_text = scrolledtext.ScrolledText(main_frame, width=80, height=15, state=tk.DISABLED, wrap=tk.WORD)
    log_text.pack(pady=5, fill=tk.BOTH, expand=True)

    # Botões de Ação
    action_buttons_frame = tk.Frame(main_frame)
    action_buttons_frame.pack(pady=10)
    tk.Button(action_buttons_frame, text="Logout", command=lambda: logout_and_close(main_app_window)).pack(side=tk.LEFT, padx=5)
    tk.Button(action_buttons_frame, text="Sair do Aplicativo", command=lambda: on_closing_main_app(main_app_window)).pack(side=tk.LEFT, padx=5)
    ...
    if monitoring_path and local_archive_folder and LOGGED_IN_USER_ID and LOGGED_IN_USER_EMAIL:
        ...
        add_log("Selecione as pastas de monitoramento e arquivamento para iniciar o monitor.", "info")
